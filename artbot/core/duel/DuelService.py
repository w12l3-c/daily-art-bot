"""
chain.py
This file contains functions related to the daily art duel aspect of the bot
"""
...
class DuelService:
    def __init__(self) -> None:
        self._tracked_users_ref = {}

    def get_tracked_users(self):
        return self._tracked_users_ref
    
    def set_tracked_users_reference(self, tracked_users_ref):
        self._tracked_users_ref = tracked_users_ref
    
    async def duel_type_autocomplete(self, 
    ...
    async def user_autocomplete(self, 
        interaction: discord.Interaction,
        current: str,
    ) -> list[app_commands.Choice[str]]:
    ...
    async def validate_duel_challenge(self, challenger_id: int, target_id: int, duel_type: str, period: int) -> tuple[bool, str]:
    ...
    async def create_duel_challenge(self, challenger_id: int, target_id: int, duel_type: str, period: int, target_time: str = None) -> str:
    ...
    def format_duel_description(self, duel_data: dict) -> str:
    ...
    async def duel_challenge_command(self, interaction: discord.Interaction, opponent: str, duel_type: str, period: int, target_time: str = None):
        ...
        is_valid, validation_message = await self.validate_duel_challenge(challenger_id, target_id, duel_type, period)
        
        if not is_valid:
            from artbot.shared import send_interaction_message
            await send_interaction_message(interaction, validation_message, ephemeral=True)
            return
        
        # Create the duel
        result = await self.create_duel_challenge(challenger_id, target_id, duel_type, period, target_time)
        ...
        description = self.format_duel_description(duel_data)
        ...
        from artbot.shared import send_interaction_message
        await send_interaction_message(interaction, challenge_message)
        logger.info(f"Duel challenge sent: {duel_data['challenger_name']} challenged {duel_data['target_name']}")
    
    async def duel_respond_command(self, interaction: discord.Interaction, response: str):
        ...
        if response.lower() == "accept":
            ...
            description = self.format_duel_description(user_duel)
            accept_message = (
                f"⚔️ **DUEL ACCEPTED** ⚔️\n\n"
            ...
            from artbot.shared import send_interaction_message
            await send_interaction_message(interaction, accept_message)
            ...
            from artbot.shared import send_interaction_message
            await send_interaction_message(interaction, deny_message)
    ...
    async def duel_status_command(self, interaction: discord.Interaction):
        ...
        if pending_as_challenger:
            ...
            for duel in pending_as_challenger:
                description = self.format_duel_description(duel)
        ...
        from artbot.shared import send_interaction_message
        await send_interaction_message(interaction, status_message, ephemeral=True)
    
    async def duel_leaderboard_command(self, interaction: discord.Interaction):
        ...
        from artbot.shared import send_interaction_message
        await send_interaction_message(interaction, leaderboard_message)
    
    async def duel_forfeit_command(self, interaction: discord.Interaction):
        ...
        from artbot.shared import send_interaction_message
        await send_interaction_message(interaction, forfeit_message)
        logger.info(f"Duel forfeited: {forfeiter_name} forfeited to {winner_name}")
    
    def get_duel_rankings(self):
    ...
    def determine_duel_winner(self, duel_data):
    ...
    async def announce_duel_completion(self, duel_data, bot=None, allowed_channels=None, announcement_channel=None):
        ...
        winner_name, loser_name, challenger_score, target_score, is_tie = self.determine_duel_winner(duel_data)
            ...
            try:
                ...
                if channel_id:
                    ...
                    if channel:
                        ...
                        await send_discord_message(channel, completion_message)
                        logger.info(f"Successfully posted duel completion to channel: {duel_data['challenger_name']} vs {duel_data['target_name']}")
                        print("Duel completion message posted to channel!")
                    else:
                        logger.error(f"Could not find channel {channel_id}")
                else:
                    logger.error("No announcement channel or allowed channels available")
            except Exception as e:
                logger.error(f"Failed to post duel completion to channel: {e}")
        else:
            logger.warning("No bot instance provided to announce_duel_completion")
        
        return completion_message
    
    # Helper function to update duel progress (called from main bot when art is submitted)
    def update_duel_progress(self, user_id: int, submission_time: datetime = None):
    ...
    async def cleanup_expired_duels(self, bot=None, allowed_channels=None, announcement_channel=None):
        ...
        for duel_id in completed_active:
            ...
            await self.announce_duel_completion(duel, bot, allowed_channels, announcement_channel)
            
            del active_duels[duel_id]
        
        return len(expired_pending) + len(completed_active)
    
    def get_duel_data(self):
    ...
    def load_duel_data(self, duel_data):
...
default_duel_service = DuelService()


def get_tracked_users():
    return default_duel_service.get_tracked_users()


def set_tracked_users_reference(tracked_users_ref) -> None:
    default_duel_service.set_tracked_users_reference(tracked_users_ref)


async def duel_type_autocomplete(interaction: discord.Interaction, current: str) -> list[app_commands.Choice[str]]:
    return await default_duel_service.duel_type_autocomplete(interaction, current)


async def user_autocomplete(interaction: discord.Interaction, current: str) -> list[app_commands.Choice[str]]:
    return await default_duel_service.user_autocomplete(interaction, current)


async def validate_duel_challenge(challenger_id: int, target_id: int, duel_type: str, period: int) -> tuple[bool, str]:
    return await default_duel_service.validate_duel_challenge(challenger_id, target_id, duel_type, period)


async def create_duel_challenge(challenger_id: int, target_id: int, duel_type: str, period: int, target_time: str = None) -> str:
    return await default_duel_service.create_duel_challenge(challenger_id, target_id, duel_type, period, target_time)


def format_duel_description(duel_data: dict) -> str:
    return default_duel_service.format_duel_description(duel_data)


async def duel_challenge_command(interaction: discord.Interaction, opponent: str, duel_type: str, period: int, target_time: str = None) -> None:
    await default_duel_service.duel_challenge_command(interaction, opponent, duel_type, period, target_time)


async def duel_respond_command(interaction: discord.Interaction, response: str) -> None:
    await default_duel_service.duel_respond_command(interaction, response)


async def duel_status_command(interaction: discord.Interaction) -> None:
    await default_duel_service.duel_status_command(interaction)


async def duel_leaderboard_command(interaction: discord.Interaction) -> None:
    await default_duel_service.duel_leaderboard_command(interaction)


async def duel_forfeit_command(interaction: discord.Interaction) -> None:
    await default_duel_service.duel_forfeit_command(interaction)


def get_duel_rankings():
    return default_duel_service.get_duel_rankings()


def determine_duel_winner(duel_data):
    return default_duel_service.determine_duel_winner(duel_data)


async def announce_duel_completion(duel_data, bot=None, allowed_channels=None, announcement_channel=None) -> None:
    await default_duel_service.announce_duel_completion(duel_data, bot, allowed_channels, announcement_channel)


def update_duel_progress(user_id: int, submission_time: datetime = None):
    return default_duel_service.update_duel_progress(user_id, submission_time)


async def cleanup_expired_duels(bot=None, allowed_channels=None, announcement_channel=None) -> int:
    return await default_duel_service.cleanup_expired_duels(bot, allowed_channels, announcement_channel)


def get_duel_data():
    return default_duel_service.get_duel_data()


def load_duel_data(duel_data) -> None:
    default_duel_service.load_duel_data(duel_data)


def clear_all_duels():
    return default_duel_service.clear_all_duels()
