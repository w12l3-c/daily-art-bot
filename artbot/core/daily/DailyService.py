...
import subprocess
import random
import artbot.debug.DebugCommands as forced_events
from artbot.core.duel.DuelService import update_duel_progress, get_duel_rankings, get_duel_data
from artbot.core.chain.ChainService import process_chain_submission, get_chain_data
from artbot.core.challenge.ChallengeService import end_challenge, is_challenge_active
import asyncio
...
class DailyService:
    ...
    def message_has_media(self, message):
    ...
    async def on_message_daily(self, message):
        ...
        has_media = self.message_has_media(message)
        ...
        if has_media:
            ...
            if message.author.id not in shared.tracked_users:
                ...
                if member is None:
                    logger.info(
                        "Auto-track role check cache miss | author_id=%s author_name=%s guild_id=%s",
                        message.author.id,
                        message.author.name,
                    ...
                    try:
                        ...
                        logger.warning(
                            "Auto-track role check fetch failed | author_id=%s error=%s",
                            message.author.id,
                            e,
                        )

                has_dailies_role = shared.has_named_role(member, ["dailies challenger"])
                role_diagnostic = shared.describe_member_roles(member)
                diagnostic_message = (
                    "Auto-track role check | "
                    f"author_id={message.author.id} "
                    f"author_name={message.author.name} "
                    f"has_dailies_challenger={has_dailies_role} "
                    f"{role_diagnostic}"
                )
    ...
    # Edit the seconds 
    async def send_daily_art_message(self):
        now = shared.now_et()
        ...
        if (now.hour == 0 and now.minute <= 1 and shared.last_daily_message_day != shared.now_et_day_str()) or forced_events.forced_daily: 
            ...
            messages = self.build_reminder_message(3)
            ...
            if shared.current_day >= shared.season_days + 1:
                ...
                if channel:
                    ...
                    MAX_LENGTH = shared.DISCORD_SAFE_MESSAGE_LIMIT
                    ZERO_WIDTH_BREAK = shared.DISCORD_MARKDOWN_BREAK
                    ...
                    if len(results_message) > MAX_LENGTH:
                        ...
                        if badge:
                            await shared.send_discord_message(channel, results_message, file=badge)
                        else:
                            await shared.send_discord_message(channel, results_message)
                    
                    # Send badge message separately if we split the main message
                    if len(results_message) > MAX_LENGTH:
                        await shared.send_discord_message(channel, f"🎨 **The {shared.season_theme} badge** 🎨")
                    else:
                        await shared.send_discord_message(channel, f"🎨 **The {shared.season_theme} badge** 🎨")
                else:
                ...
                if channel:
                    await shared.send_discord_message(channel, season_message)
                else:
    ...
    def build_reminder_message(self, num = 1):        
        ...
        # Graveyard display disabled for now. Keep deceased state logic intact.
        # message += "\n\n**🪦 Deceased:**\n"
        #
        ...
        split_messages = []
        for message in messages:
            split_messages.extend(shared.split_discord_message(message))
        return split_messages
    
    async def ping_jailed_users(self):
        now = shared.now_et()
        ...
        if now.hour == 22 and now.minute <= 1  and shared.last_warning_1_day != shared.now_et_day_str(): 
            ...
            messages = self.build_reminder_message(1)
            ...
            shared.last_warning_2_day = shared.now_et_day_str()
            messages = self.build_reminder_message(2)
...
default_daily_service = DailyService()


def reset_user_stats() -> None:
    default_daily_service.reset_user_stats()


def message_has_media(message) -> bool:
    return default_daily_service.message_has_media(message)


async def on_message_daily(message) -> None:
    await default_daily_service.on_message_daily(message)


@tasks.loop(time=time(hour=0, minute=0, second=0, microsecond=0, tzinfo=ZoneInfo("America/Toronto")))
async def send_daily_art_message() -> None:
    await default_daily_service.send_daily_art_message()


def build_reminder_message(num: int = 1) -> list[str]:
    return default_daily_service.build_reminder_message(num)


@tasks.loop(minutes=1)
async def ping_jailed_users() -> None:
    await default_daily_service.ping_jailed_users()
