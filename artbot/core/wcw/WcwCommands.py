...
from artbot.CommandChecks import requires_wcw_permission

# ------------------------------------------------ Mod commands ------------------------------------------------


class WcwCog(commands.Cog):
    def __init__(self, bot_instance, app_context) -> None:
        self.bot = bot_instance
        self.app_context = app_context

    @app_commands.command(name="wcw_edit_user", description="Edit a user in WCW. (Must be mod or warden)")
    ...
    @requires_wcw_permission()
    async def wcw_edit_user(self, 
    ...
    @app_commands.command(name="wcw_add", description="Add a user to WCW. (Must be mod or warden)")
    @app_commands.describe(user="Select a user")
    @requires_wcw_permission()
    async def wcw_add(self, interaction: discord.Interaction, user: discord.User):
    ...
    @app_commands.command(name="wcw_remove", description="Remove a user from WCW. (Must be mod or warden)")
    @app_commands.describe(user="Select a user")
    @requires_wcw_permission()
    async def wcw_remove(self, interaction: discord.Interaction, user: discord.User):
        if wcw.is_active_member(user):
    ...
    @app_commands.command(name="wcw_excuse", description="Excuse a user from this week's WCW. (Must be mod or warden)")
    @app_commands.describe(user="Select a user")
    @requires_wcw_permission()
    async def wcw_excuse(self, interaction: discord.Interaction, user: discord.User):
        if wcw.is_active_member(user):
    ...
    # DEBUG
    @app_commands.command(name="wcw_debug", description="E")
    @app_commands.describe(debug="int")
    @requires_wcw_permission()
    async def wcw_debug(self, interaction: discord.Interaction, debug: int = 0):
    ...
    @app_commands.command(name="wcw_edit_week", description="Edit the week number")
    @app_commands.describe(week_number="int")
    @requires_wcw_permission()
    async def wcw_edit_week(self, interaction: discord.Interaction, week_number: int = 0):
    ...
    @app_commands.command(name="wcw_join", description="Join WCW!")
    async def wcw_join(self, interaction: discord.Interaction):
    ...
    @app_commands.command(name="wcw_leave", description="Leave WCW. Your stats will remain archived.")
    async def wcw_leave(self, interaction: discord.Interaction):
        user = interaction.user
        if wcw.is_active_member(user):
            await wcw.remove_user(user)
    ...
    @app_commands.command(name="wcw_partake", description="Enlighten thyself of the beauty of the written word.")
    async def wcw_partake(self, interaction: discord.Interaction):
        user = interaction.user
        if wcw.is_active_member(user):
    ...
    @app_commands.command(name="wcw_withdraw", description="Remove thyself from our commune, and regress to oblivion and obscurity.")
    async def wcw_withdraw(self, interaction: discord.Interaction):
        user = interaction.user
        if wcw.is_active_member(user):
            await wcw.remove_user(user)
    ...
    @app_commands.command(name="wcw_goal", description="Set your weekly writing goal! It can be a word count, a description or something else.")
    @app_commands.describe(goal="Your weekly goal! This is not enforced.")
    async def wcw_goal(self, interaction: discord.Interaction, goal: str):  
        user = interaction.user
        if wcw.is_active_member(user):
    ...
    @app_commands.command(name="wcw_find_submissions", description="Find a user or all user's submissions for this week!")
    @app_commands.describe(user="Select a user, or leave blank to view all")
    async def wcw_find_submissions(self, interaction: discord.Interaction, user: discord.User = None):  
        ...
        await shared.send_interaction_message(interaction, message, ephemeral=True)
    
    
    
    @app_commands.command(name="wcw_ping", description="Set your ping preference! Reminders are at 9 AM and 9 PM every Wednesday")
    @app_commands.describe(ping="Whether or not you want pings")
    async def wcw_ping(self, interaction: discord.Interaction, ping: bool):  
        user = interaction.user
        if wcw.is_active_member(user):
    ...
    @app_commands.command(name="wcw_view_stats", description="View your own or someone else's stats!")
    @app_commands.describe(user="The user you want to view the stats of. Leave blank for yourself")
    async def wcw_view_stats(self, interaction: discord.Interaction, user: discord.User = None):
        if user is None:
            user = interaction.user
