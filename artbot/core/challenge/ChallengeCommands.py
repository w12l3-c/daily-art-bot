...
from artbot.shared import bot, logger, confirmation_prompt, save_data_task
import random
from artbot.CommandChecks import requires_challenge_mod
...
class ChallengeCog(commands.Cog):
    ...
    @app_commands.command(name="challenge_start", description="Start a challenge! (Must be mod)")
    ...
    @requires_challenge_mod()
    async def challenge_start(self, interaction: discord.Interaction, theme: str, threshold: int = -1, length: int = 7, channel: discord.TextChannel = None):
        if threshold == -1:
            threshold = length
    ...
    @app_commands.command(name="challenge_cancel", description="Cancel a challenge. (Must be mod)")
    @requires_challenge_mod()
    async def challenge_cancel(self, interaction: discord.Interaction):
    ...
    @app_commands.command(name="challenge_add", description="Add a user to the Challenges! (Must be mod)")
    @app_commands.describe(user="Select a user")
    @requires_challenge_mod()
    async def challenge_add(self, interaction: discord.Interaction, user: discord.User):
    ...
    @app_commands.command(name="challenge_remove", description="Remove a user from Challenges. (Must be mod)")
    @app_commands.describe(user="Select a user")
    @requires_challenge_mod()
    async def challenge_remove(self, interaction: discord.Interaction, user: discord.User):
        if user.id not in shared.tracked_users:
            await interaction.response.send_message(f"{user.display_name} is not in dailies! Please register them before adding them to a challenge.", ephemeral=True)
            return
    ...
    @app_commands.command(name="challenge_edit", description="Edit or view an ongoing challenge's attributes. (Must be mod)")
    ...
    @app_commands.command(name="challenge_edit_user", description="Edit or view a user's attributes. (Must be mod)")
    @app_commands.describe(
        user="Select a user",
    ...
    @requires_challenge_mod()
    async def challenge_edit_user(self, 
        interaction: discord.Interaction, 
        user: discord.User,
    ...
    @app_commands.command(name="challenge_join", description="Join Challenges!")
    async def challenge_join(self, interaction: discord.Interaction):
    ...
    @app_commands.command(name="challenge_leave", description="Leave Challenges.")
    async def challenge_leave(self, interaction: discord.Interaction):
        if interaction.user.id not in shared.tracked_users:
            await interaction.response.send_message(f"You are not in the daily art tracking system! Please join that first.", ephemeral=True)
            return
    ...
    @app_commands.command(name="challenge_stats", description="See your challenge stats!")
    ...
    @app_commands.command(name="challenge_find_submissions", description="Find today's challenge submission of a user or all users!")
    @app_commands.describe(user="Select a user, or leave blank to view all")
    async def challenge_find_submissions(self, interaction: discord.Interaction, user: discord.User = None):
...
ChallengeCommands = ChallengeCog
